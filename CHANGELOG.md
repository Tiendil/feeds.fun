...
## 1.29.3 on 2026-09-04

### Changes

- ff-710 — The "undo read" button moved from the sidebar to the top toolbar.
- ff-711 — Improved Discovery view and toolbar:
  - The Discover Feeds page is now split into clear "Add a feed" and "Import OPML" sections, with handy links to jump between them.
  - New tips explain how to choose feed URLs and avoid duplicate news or wasting tokens and API-key quota.
  - You can now add a feed from the News or Feeds page. The Feeds toolbar also has quick access to importing and exporting OPML files.
  - You can subscribe to a collection right from its public page.
  - Public collection pages now show the collection description with the welcome message and explain why news is always sorted by publication date.

## 1.29.2 on 2026-08-31

- ff-707 (gh-480) — unified display of entry published dates.

## 1.29.1 on 2026-08-31
...
## 1.27.1 on 2026-05-14

- Added `snapshot_at` to metrics output by `ffun metrics`, to allow easier building of derived metrics.

## 1.27.0 on 2026-05-14
...
## 1.25.1 on 2026-03-23

- ff-679 — Improved calculation of the age of news entry, to prevent some cases of overspending tokens on processing old entries that look like new ones because of the wrong value of publish date.

## 1.25.0 on 2026-03-18
...
## 1.24.2 on 2026-03-16

- ff-676 — Stopped updating the date of news entries publishing on each feed load because it caused constant entries reordering for feeds in broken or unconventional format.

## 1.24.1 on 2026-03-16

- ff-675 — Use `updated` date as a fallback for `published` date in RSS/ATOM feeds when `published` date is missing.

## 1.24.0 on 2026-03-15
...
## 1.23.1 on 2026-03-14

- ff-673 — Fixed removing orphaned entries behaviour in the case when the orphaned entry is concurrently linked to a feed.

## 1.23.0 on 2026-03-13
...
## 1.22.8 on 2026-03-06

- ff-330 — Removed "free of charge" statement from the footer.
- ff-588 — Support maintenance mode for the frontend.

## 1.22.7 on 2026-03-04
...
## 1.22.4 on 2026-01-31

- ff-630 - Improved performance of feeds discovery by reducing the number of processed URLs.
