...
from ffun.llms_framework.entities import ChatResponse, LLMConfiguration, Provider, LLMCollectionApiKey, LLMGeneralApiKey, LLMApiKey
from ffun.llms_framework.provider_interface import ProviderTest, ChatResponseTest
...
class TestProcessor:
    ...
    def llm_processor(self, llm_config: LLMConfiguration) -> Processor:
        return Processor(name='test-llm-processor',
    ...
    def test_extract_tags(self,
        ...
        assert tags == [ProcessorTag(raw_uid="tag-1"),
                        ProcessorTag(raw_uid="tag-2"),
                        ProcessorTag(raw_uid="tag-3")]
    ...
    @pytest.mark.asyncio
    async def test_process__has_api_key_found(self, llm_processor: Processor, cataloged_entry: Entry, fake_llm_api_key: LLMApiKey) -> None:

        entry = cataloged_entry.replace(title="@tag-1 @tag-2",
                                        body="@tag-3 @tag-2")
