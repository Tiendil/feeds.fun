...
def apply_step(conn: Connection[dict[str, Any]]) -> None:
    ...
    for processor_id, categories in [(1, 'network-domain'),
                                     (2, 'feed-tag'),
                                     (5, 'special')]:
        cursor.execute(sql.format(condition='p.processor_id = %(processor_id)s'),
                       {'processor_id': processor_id, 'categories': categories})

    cursor.execute(sql.format(condition='p.processor_id NOT IN (1, 2, 5)'),
