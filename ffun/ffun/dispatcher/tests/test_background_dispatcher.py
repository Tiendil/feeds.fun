...
class TestEntriesDispatcher:
    ...
    async def test_single_run__uses_configured_chunk(self, mocker: MockerFixture) -> None:
        ...
        dispatcher = EntriesDispatcher(processor_ids=[101, 102], chunk=5, name="test_dispatcher", delay_between_runs=1)

        await dispatcher.single_run()

        dispatch_entries.assert_awaited_once_with(processor_ids=(101, 102), limit=5)
    ...
    async def test_single_run__uses_default_chunk_from_settings(self, mocker: MockerFixture) -> None:
        ...
        dispatcher = EntriesDispatcher(processor_ids=[101, 102], name="test_dispatcher", delay_between_runs=1)

        await dispatcher.single_run()

        dispatch_entries.assert_awaited_once_with(processor_ids=(101, 102), limit=17)
