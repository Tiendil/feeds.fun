...
def apply_step(conn: Connection[dict[str, Any]]) -> None:
    ...
    RENAME COLUMN published_at TO ingested_at
    """)
    cursor.execute("""
    UPDATE l_feeds_to_entries
    SET ingested_at = created_at
    """)
    cursor.execute("""
    DROP INDEX IF EXISTS l_feeds_to_entries_feed_id_published_at_created_at_entry_id_idx
    """)
    cursor.execute("""
...
def rollback_step(conn: Connection[dict[str, Any]]) -> None:
    ...
    cursor.execute("""
    DROP INDEX IF EXISTS l_feeds_to_entries_feed_id_ingested_at_idx
    """)
    cursor.execute("""
    DROP INDEX IF EXISTS l_feeds_to_entries_feed_id_created_at_entry_id_idx
    """)
    cursor.execute("""
    ALTER TABLE l_feeds_to_entries
    RENAME COLUMN ingested_at TO published_at
    """)
    cursor.execute("""
