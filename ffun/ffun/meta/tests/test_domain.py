...
class TestMergeFeeds:
    ...
    @pytest.mark.asyncio
    async def test_no_entries_in_source_feed(self,
                                             loaded_feed_id: uuid.UUID,
                                             another_loaded_feed_id: uuid.UUID) -> None:
        await merge_feeds(loaded_feed_id, another_loaded_feed_id)

    @pytest.mark.asyncio
    async def test_move_entries_to_an_empty_feed(self,
                                                 loaded_feed_id: uuid.UUID,
                                                 another_loaded_feed_id: uuid.UUID,
                                                 fake_processor_id: int,
                                                 another_fake_processor_id: int,
        ...
        loaded_entries = await l_domain.get_entries_by_filter(feeds_ids=[another_loaded_feed_id],
                                                              limit=100)
    ...
    @pytest.mark.asyncio
    async def test_merge_entries(self,
                                 loaded_feed_id: uuid.UUID,
                                 another_loaded_feed_id: uuid.UUID,
                                 fake_processor_id: int,
                                 another_fake_processor_id: int,
        ...
        assert loaded_entries == [entry.replace(feed_id=another_loaded_feed_id,
                                                id=another_entries[i].id,
                                                cataloged_at=another_entries[i].cataloged_at)
                                  for i, entry in enumerate(entries)]

        tags = await o_domain.get_tags_for_entries([entry.id for entry in loaded_entries])

        assert tags == {another_entries[0].id: {tag_a.raw_uid},
                        another_entries[1].id: {tag_a.raw_uid, tag_b.raw_uid, tag_c.raw_uid},
                        another_entries[2].id: {tag_c.raw_uid}}

    @pytest.mark.asyncio
    async def test_move_and_merge(self,
                                  loaded_feed_id: uuid.UUID,
                                  another_loaded_feed_id: uuid.UUID,
                                  fake_processor_id: int,
                                  another_fake_processor_id: int,
        ...
        # fill feed 2
        await o_domain.apply_tags_to_entry(entry_id=another_entries[0].id,
                                           processor_id=fake_processor_id,
                                           tags=[tag_c])
