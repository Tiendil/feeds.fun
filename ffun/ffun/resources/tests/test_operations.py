...
class TestInitializeResource:
    ...
    async def test_new_record(self,
                              ...
                              interval_started_at: datetime.datetime) -> None:
        async with TableSizeDelta('r_resources', delta=1):
            resource = await initialize_resource(user_id=internal_user_id,
                                                 kind=_kind,
        ...
        async with TableSizeNotChanged('r_resources'):
            loaded_resource = await load_resource(user_id=internal_user_id,
                                                  kind=_kind,
                                                  interval_started_at=interval_started_at)
    ...
    @pytest.mark.asyncio
    async def test_do_not_reinitialized_if_exists(self,
                                                  internal_user_id: uuid.UUID,
                                                  interval_started_at: datetime.datetime) -> None:
...
class TestLoadResources:
    ...
    @pytest.mark.asyncio
    async def test_initialize_if_not_found(self,
                                           internal_user_id: uuid.UUID,
        ...
        async with TableSizeDelta('r_resources', delta=1):
            resources = await load_resources(user_ids=[internal_user_id, another_internal_user_id],
                                             kind=_kind,
                                             interval_started_at=interval_started_at)
...
class TestTryToReserve:
    ...
    @pytest.mark.asyncio
    async def test_for_not_existed_resource(self, amount: int, internal_user_id: uuid.UUID, interval_started_at: datetime.datetime) -> None:
        result = await try_to_reserve(user_id=internal_user_id,
                                      kind=_kind,
                                      interval_started_at=interval_started_at,
                                      amount=amount,
                                      limit=100)
    ...
    @pytest.mark.asyncio
    async def test_for_existed_resource(self, internal_user_id: uuid.UUID, interval_started_at: datetime.datetime) -> None:
        result = await try_to_reserve(user_id=internal_user_id,
                                      kind=_kind,
                                      interval_started_at=interval_started_at,
                                      amount=1,
...
class TestConvertReservedToUsed:
    ...
    @pytest.mark.parametrize('reserved, converted_reserved, converted_used, expected_reserved, expected_used',
                             [(13, 13, 9, 0, 9),
                              (13, 13, 13, 0, 13),
                              (77, 14, 13, 63, 13)])
    @pytest.mark.asyncio
    async def test_converted(self,  # noqa: CFQ002
                             internal_user_id: uuid.UUID,
        ...
        await convert_reserved_to_used(user_id=internal_user_id,
                                       kind=_kind,
                                       interval_started_at=interval_started_at,
...
class TestLoadResourceHistory:
    ...
    async def test_with_history(self, internal_user_id: uuid.UUID, another_internal_user_id: uuid.UUID) -> None:
        ...
        await try_to_reserve(user_id=another_internal_user_id,
                                kind=_kind,
