...
2. Update your custom `tag_processors.toml` configs to use explicit processor routes.
   - Every processor now needs a `routes` list. Move old top-level `allowed_for_collections` and
     ...
     For simple processors, use one route such as `id = "default"`.
   ...
   - To replace an old general API key, add a route with the old `allowed_for_collections` and
     `allowed_for_users` values and `api_key = "<old general_api_key>"`.
